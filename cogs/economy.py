...
#
# A silly little module for users to make and loose $$$. By Cal_9000
...
class Economy:
    ...
    async def register(self, ctx):
        """Registers an account at the Cal bank"""
        ...
        if user.id not in self.bank:
            ...
            await self.bot.say("{} You already have an account at the Cal bank.".format(user.mention))
    ...
    async def balance(self, ctx, user : discord.Member=None):
        ...
        if not user:
            ...
            if self.account_check(user.id):
                ...
                await self.bot.say("{} You don't have an account at the Cal bank. Type {}bank register to open one.".format(user.mention, ctx.prefix))
        else:
            if self.account_check(user.id):
